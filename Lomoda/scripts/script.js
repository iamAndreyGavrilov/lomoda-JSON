...
const disableScroll = () => {
    const widtchScroll = window.innerWidth - document.body.offsetWidth;
...
const getData = async() => {
    ...
    if (data.ok) {
        ...
        throw new Error(`Данный не были получены, ошибка ${data.status} ${data.statusText}`);
    }
};

//

const getGoods = (callback, prob, value) => {
    getData()
        .then(data => {
            if (value) {
                callback(data.filter(item => item[prob] === value));
            } else {
                callback(data);
            }
        })
...
try {
    ...
    const renrerCardGood = ([{ id, brand, name, cost, color, sizes, photo }]) => {
...
if (getLocalStorage().some(item => item.id === id)) {
    ...
    getGoods(renrerCardGood, 'id', hash);
} catch (error) {
    console.warn(error);
}